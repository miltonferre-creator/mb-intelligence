  ...
  function publicationCenter() {
    ...
    return `
      ...
      <section class="panel">
        ...
        ${periods.length ? MBI.ui.table(["Competência", "Faturamento", "Impostos", "Folha", "Resultado", "Margem", "Ações"], periods.map((row) => [MBI.ui.escape(row.label), MBI.ui.money(row.revenue), MBI.ui.money(row.taxes), MBI.ui.money(row.payroll), MBI.ui.money(row.result), `${row.margin}%`, `<button class="btn btn-soft btn-mini" type="button" data-action="edit-finance-period" data-client-id="${MBI.ui.escape(client.id)}" data-competence="${MBI.ui.escape(row.competence)}">${MBI.ui.icon("pencil")} Editar</button>`])) : `<div class="empty-lock">${MBI.ui.icon("calendar")}<h3>Nenhum período registrado</h3><p>Clique em "Lançar dados do mês" para criar o primeiro histórico.</p></div>`}
      </section>
